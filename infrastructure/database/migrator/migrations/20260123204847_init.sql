...
CREATE FUNCTION is_valid_unit_archetype(unit_archetype text) RETURNS boolean AS $$
BEGIN
...
CREATE FUNCTION is_valid_troop(troop integer[]) RETURNS boolean AS $$
BEGIN
...
CREATE FUNCTION is_valid_battle_log(log jsonb) RETURNS boolean AS $$
BEGIN
...
CREATE FUNCTION prevent_unremove() RETURNS TRIGGER AS $$
BEGIN
...
CREATE FUNCTION prevent_read_whisper_content_update() RETURNS TRIGGER AS $$
BEGIN
...
CREATE FUNCTION notify_outbox_insertion() RETURNS TRIGGER AS $$
BEGIN
...
CREATE TABLE public.protagonists_chronicles (
    "protagonist_id" uuid,
    "chronicle_id" uuid,
...
-- CHAPTERS VIEWS
CREATE TABLE public.chapters_views (
    "chapter_id" uuid,
    "user_id" uuid,
    "inserted_at" timestamp NOT NULL DEFAULT (CURRENT_TIMESTAMP),
...
CREATE PROCEDURE register_kingdom_and_leader(
    p_kingdom_id uuid,
    p_leader_id uuid,
    p_user_id uuid,
...
-- ROLEPLAY
CREATE VIEW roleplay.chapters AS
    SELECT *
    FROM public.users;
CREATE VIEW roleplay.chronicles AS
    SELECT *
    FROM public.chronicles;
CREATE VIEW roleplay.kingdoms AS
    SELECT id, name, fame, slug
    FROM public.kingdoms;
CREATE VIEW roleplay.players AS
    SELECT id, nickname, profile_picture, slug
    FROM public.users;
CREATE VIEW roleplay.protagonists AS
    SELECT *
    FROM public.protagonists;
CREATE VIEW roleplay.whispers AS
    SELECT *
    FROM public.whispers;

-- SOCIAL
CREATE VIEW social.boards AS
    SELECT *
    FROM public.boards;
CREATE VIEW social.posts AS
    SELECT *
    FROM public.posts;
CREATE VIEW social.shouts AS
    SELECT *
    FROM public.shouts;
CREATE VIEW social.topics AS
    SELECT *
    FROM public.topics;
CREATE VIEW social.users AS
    SELECT *
    FROM public.users;

-- SOVEREIGNTY
CREATE VIEW sovereignty.battles AS
    SELECT *
    FROM public.battles;
CREATE VIEW sovereignty.kingdoms AS
    SELECT *
    FROM public.kingdoms;
CREATE VIEW sovereignty.missives AS
    SELECT *
    FROM public.missives;
CREATE VIEW sovereignty.notables AS
    SELECT *
    FROM public.protagonists;
CREATE VIEW sovereignty.players AS
    SELECT id, nickname, profile_picture, slug
    FROM public.users;

-- TECHNICAL
CREATE VIEW technical.outbox AS
    SELECT *
    FROM public.outbox;
...
COMMENT ON CONSTRAINT "fk_kingdoms_leader_id" ON "kingdoms"
IS 'Deferrable to allow registering a kingdom with its leader protagonist; \
   see register_kingdom_and_leader procedure';
COMMENT ON CONSTRAINT "fk_kingdom_has_leader" ON "kingdoms" IS 'Deferrable to allow changing leader';

-- PROTAGONISTS
COMMENT ON CONSTRAINT "fk_protagonists_kingdom_id" ON "protagonists"
IS 'Deferrable to allow registering a kingdom with its leader protagonist; \
   see register_kingdom_and_leader procedure';
...
DROP TYPE IF EXISTS outbox_message_status CASCADE;
