defmodule PlatformWeb.BattleController do
  ...
  def attack(%{assigns: %{current_account: user}} = conn, %{
        "atk_kingdom_id" => atk_kingdom_id,
        "def_kingdom_id" => def_kingdom_id
      }) do
    with {:ok, atk_kingdom} <- SovereigntyRepo.get_kingdom(atk_kingdom_id),
         ...
         {:ok, battle} <-
           SovereigntyRepo.save_battle_result(battle_outcome, atk_kingdom, def_kingdom) do
      conn
      |> put_view(PlatformWeb.Views.BattleJSON)
      |> render(:show, battle)
    else
      {:error, :unauthorized} ->
        error(conn, :forbidden, "User cannot attack with a foreign kingdom")

      {:error, :inactive_kingdom} ->
        error(conn, :forbidden, "One of the kingdoms is not active")

      {:error, :not_found} ->
        error(conn, :not_found, "Target kingdom does not exist")

      {:error, reason} ->
        error(conn, :bad_request, reason)
    end
  end
