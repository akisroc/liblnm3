defmodule PlatformInfra.Database.Accounts do
  ...
  alias PlatformInfra.Database.Schemas.Session
  ...
  def register_user(%{
    ...
    |> Multi.insert(:kingdom, fn %{user: user, protagonist: leader} ->
      Kingdom.create_changeset(%Kingdom{}, %{
        name: kingdom_name,
        user_id: user.id,
        leader_id: leader.id
      })
  ...
  def delete_expired_sessions do
    {count, _} = Repo.delete_all(from s in Session, where: s.expires_at < fragment("now()"))
