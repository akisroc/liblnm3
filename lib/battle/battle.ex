defmodule Battle do
  ...
  # Theoric raw kills are straight arithmetic based on
  # power–defense–killrate, then we apply successive ratios:

  # Friction: the less the stricken, the harder they are to kill.
  # We use hyperbolic tangent with 5.0 as configurable magic number.

  # Luck: Damages spread between 0.8 to 1.2 ratio.

  # For conciseness:
  # u1 => Striking unit
  # u2 => Stricken unit
  defp kill_count(u1, u2) do
    raw_kills = u1.count * u1.power * u1.kill_rate / u2.defense

    friction_ratio = :math.tanh(u2.count * 5.0 / max(1, u1.count - u2.count))

    {min_spread, max_spread} = {0.8, 1.2}
    luck_ratio = min_spread + :rand.uniform() * (max_spread - min_spread)

    raw_kills * friction_ratio * luck_ratio |> trunc() |> min(u2.count)
  end
